...
|---|---|---|---|
| **enrollment** | domain + adapter(jOOQ) + controller + テスト | domain model（アダプタ層なし） | record + JsonDecoder + behavior + Encoder + controller + テスト |
| **joboffer** | sealed record + mapValidator + STI永続化 + controller + テスト | フラットForm + カスタムValidator + controller + テスト | sealed record + discriminateデコーダ/エンコーダ + controller + テスト |
| **reserve** | domain + adapter(jOOQ) + controller + テスト | Form + Service + controller + テスト | record + JsonDecoder + behavior + Encoder + controller + テスト |
...
入力の境界を型で固めても、出力が手組みなら同じ問題が反対側で起きる。

**Raoh** — エンコーダ（`MapEncoders`）でレスポンスの形を定義する。デコーダと同じ判別キー・同じタグ名を共有するので、`discriminate`の構造がそのまま鏡写しになる。

```java
// デコーダ: JSON → JobOffer
JOB_OFFER_DECODER = discriminate("type", Map.of(
    "project", combine(TITLE, DESCRIPTION, OFFER_EXPIRE_DATE,
        field("settlement", SETTLEMENT_DECODER)).map(ProjectJobOffer::new),
    ...));

// エンコーダ: JobOffer → Map（鏡写し）
JOB_OFFER_ENCODER = discriminate("type",
...
デコーダがタグを読んで型を選ぶのに対し、エンコーダは値の実行時クラスから型を選んでタグを書く。エンコードした結果をデコーダに戻すと元の値に戻る（`JobOfferEncodersTest`が予算3種・精算2種・件数制限2種・契約金額5種すべてで往復を検証している）。

エンコーダを入れる前は、コントローラが`jobOffer.getClass().getSimpleName()`で`"ProjectJobOffer"`を返していた。デコーダが判別に使うタグは`"project"`なので、レスポンスを自分のデコーダに食わせ直せない。タグの定義が2箇所に分かれると、こういうズレが黙って入る。

エンコーダが作るのは`Map<String, Object>`で、JSONにするのはJacksonに渡すだけでよい。デコード側と違ってJSON専用のエンコーダが用意されていないのは、エンコーダが失敗しない全関数だからで、デコーダ側の`combine`のような誤り集約の仕組みを出力側は必要としない。

**YAVI** — ドメインオブジェクトを`ApiResponse<T>`に入れてJacksonに委ねる。出力の形はドメインの形そのままで、境界としての定義がない。ドメインモデルを変えるとレスポンスも変わる。

**Bean Validation** — `JobOfferOutput`／`ReserveTourOutput`という出力専用クラスを用意し、service層で手で詰め替える。出力の形は明示されるが、入力側の`Form`との対応関係はコードに現れない。
...
| レスポンス構築 | エンコーダをデコーダと対に定義 | ドメインをJacksonに委ねる | 出力DTOを手で詰め替え |
...
|---|---|---|---|
| バリデーション | YAVI 0.16.0 | Jakarta Bean Validation (Hibernate Validator) | Raoh 0.6.0 |
