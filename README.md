...
|---|---|---|---|
| **enrollment** | domain + adapter(jOOQ) + controller + テスト | domain model（アダプタ層なし） | — |
| **joboffer** | 多態ドメインモデル + STI永続化 + controller + テスト | フラットForm + カスタムValidator + controller + テスト | sealed record + discriminateデコーダ + controller + テスト |
...
**YAVI** — `mapValidator`通過後は具体型が手に入る。ただしバリデータがドメインオブジェクト内に同居するため、クラスが重い。
...
```java
// YAVI: mapValidatorの構造。同様に型の階層と対応するが、ドメインクラスにバリデータが同居
public abstract class JobOffer {
    static Arguments1Validator<Map<String, Object>, JobOffer> mapValidator = ...
    // 各サブタイプクラス内にも static mapValidator が定義される
}
```
...
**YAVI** — 各クラスにstatic validatorフィールド、`of()`/`parse()`ファクトリメソッドが同居。`ProjectJobOffer`は`mapTitleValidator`, `mapDescriptionValidator`, `mapOfferExpireDateValidator`, `mapValidator`の4つのstaticフィールドを持つ。
...
| バリデーション後の型安全性 | sealed recordで保証 | 型で保証される | 文字列のまま |
| ドメインモデルの軽さ | record/sealedのみ（最軽量） | バリデータ同居（重い） | Form DTO（型安全性なし） |
| 合成・再利用 | デコーダ合成が自然 | 関数合成で可能 | 別Validator切り出しが必要 |
| エラーパス | 自動構成 | 自動構成 | 手動構築 |
| I/O統合 | デコーダ内でDB参照も合成可能 | controller側で別ステップ | service側で別ステップ |
